...
final class CubeRenderer: MetalViewDelegate {
  ...
  private let _displaySemaphore = DispatchSemaphore(value: 3)
    var (_time, rotationX, rotationY, rotationZ): (Float, Float, Float, Float) = (0,0.4,0.0, 0.3)
  ...
  init(withDevice device: MTLDevice) {
    ...
    let vertices = [_Vertex(position: [-1,  0.2,  1, 1], color: [0, 1, 1, 1]), // left,  top,    back
                    _Vertex(position: [-1, -0.2,  1, 1], color: [0, 0, 1, 1]), // left,  bottom, back
                    _Vertex(position: [ 1, -0.2,  1, 1], color: [1, 0, 1, 1]), // right, bottom, back
                    _Vertex(position: [ 1,  0.2,  1, 1], color: [1, 1, 1, 1]), // right, top,    back
                    _Vertex(position: [-1,  0.2, -1, 1], color: [0, 1, 0, 1]), // left,  top,    front
                    _Vertex(position: [-1, -0.2, -1, 1], color: [0, 0, 0, 1]), // left,  bottom, front
                    _Vertex(position: [ 1, -0.2, -1, 1], color: [1, 0, 0, 1]), // right, bottom, front
                    _Vertex(position: [ 1,  0.2, -1, 1], color: [1, 1, 0, 1])] // right, top,    front
  ...
  func draw(view metalView: MetalView) {
    ...
    self._updateUniforms(drawableSize: [Float(drawableSize.width), Float(drawableSize.height)])
...
private extension CubeRenderer {
  ...
  func _updateUniforms(drawableSize: SIMD2<Float>) {
//    self._time += duration * 5
...
//    let scaleFactor: Float = 1 + 0.25*sin(5 * self._time)
    ...
    let yRotMatrix = float4x4(rotate: [0, 1, 0], angle: self.rotationY)
    let zRotMatrix = float4x4(rotate: [0, 0, 1], angle: self.rotationZ)
//    let scaleMatrix = float4x4(diagonal: [scaleFactor, scaleFactor, scaleFactor, 1])
//    let modelMatrix = yRotMatrix * (xRotMatrix * scaleMatrix)
    let modelMatrix = yRotMatrix * xRotMatrix * zRotMatrix
